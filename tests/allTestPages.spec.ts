...
//ДЗ-2 
const itemSearchMortises = page.locator('h2 > a:has-text("Mortises")').first();
...
const searchInput = page.locator('input[name="q"]');
    ...
    await expect(searchInput).toHaveAttribute('placeholder', 'Search...');
    await expect(searchInput).toHaveAttribute('type', 'search');
    await searchInput.fill('NoneItem');
    await expect(searchInput).toHaveValue('NoneItem');

const searchResultNull = page.locator('.Segment__Content').first();
    await expect(searchResultNull).toBeVisible();
    await expect(searchResultNull).toHaveText('No results could be found');
});
